...
/**
 * Capacity-creation metrics — the "growth story" companion to the savings story.
 * Answers the question: "With the same team, how much more work can we do?"
 *
 * Semantics:
 *   volumeMultiplier    — same team × (Before FTE / After FTE) = new capacity ratio.
 *                         2.5 means the same team can now handle 2.5× today's volume.
 *   additionalMonthlyVolume — today's volume × (multiplier − 1). The incremental
 *                         monthly loans/apps the team can absorb without hiring.
 *   fteCapacityFreed    — FTEs that are no longer needed to run today's volume.
 *                         Same headline number as internal.fteSaved, re-labeled
 *                         to reinforce the capacity narrative in the UI.
 *
 * All three metrics are undefined only when the After FTE count is exactly 0
 * (all direct roles eliminated) — a rare edge case for automation models.
 */
export type SaasCapacityMetrics = {
  volumeMultiplier: number;
  additionalMonthlyVolume: number;
  fteCapacityFreed: number;
  /** Monthly volume used as the denominator for capacity math (matches perLoanDenominator). */
...
export function computeSaasRoi(model: SaasModelConfig, inputs: SaasInputs): SaasResult {
  ...
  // Capacity-creation metrics (companion to the savings story).
  // Multiplier = Before-FTE / After-FTE for direct roles (excluding supervisors,
  // whose FTE count is derived and scales with direct headcount). When any
  // direct FTE remains After, this is a meaningful multiplier. When After=0
  // (all roles eliminated), multiplier is 0 (undefined semantically — the UI
  // shows a fallback).
  const directFteBefore = internal.roles.reduce((s, rb) => s + rb.fteBefore, 0);
  const directFteAfter = internal.roles.reduce((s, rb) => s + rb.fteAfter, 0);
  const volumeMultiplier = directFteAfter > 0 ? directFteBefore / directFteAfter : 0;
  const additionalMonthlyVolume =
    volumeMultiplier > 0 ? denomMonthly * (volumeMultiplier - 1) : 0;
  const capacity: SaasCapacityMetrics = {
    volumeMultiplier,
    additionalMonthlyVolume,
    fteCapacityFreed: internal.fteSaved,
    monthlyVolumeBase: denomMonthly,
  };
